...
for year in years:
    globals()["data_" + str(year)] = of.open_parquet(
        "endettes_" + str(year) + ".parquet"
    )
for year in years:
    globals()["data_" + str(year)] = globals()["data_" + str(year)].drop(
        ["montant", "ptot", "mont_hab", "perc"], axis=1
    )
    globals()["data_" + str(year)]["exer"] = year
for year in years:
    globals()["data_" + str(year)] = globals()["data_" + str(year)][
        ["exer", "siren", "group"]
    ]
    globals()["data_" + str(year)].columns = ["exer", "siren", "difficulte"]
    globals()["data_" + str(year)]["difficulte"] = globals()["data_" + str(year)][
        "difficulte"
    ].replace(to_replace=["tres endetté", " "], value=[1, 0])


for year in years:
    if year < 2018:
        globals()["data_" + str(year) + "_" + str(year + 1)] = globals()[
            "data_" + str(year)
        ].merge(right=globals()["data_" + str(year + 1)], on="siren", how="inner")
for year in years:
    if year < 2018:
        globals()["data_" + str(year) + "_" + str(year + 1)] = globals()[
            "data_" + str(year) + "_" + str(year + 1)
        ].drop(["exer_y"], axis=1)
        globals()["data_" + str(year) + "_" + str(year + 1)].columns = [
            "exer",
            "siren",
            "difficulte_anne",
            "difficulte_avenir",
...
X_train, X_test, y_train, y_test = train_test_split(
    feature, target, test_size=0.2, random_state=12
)
